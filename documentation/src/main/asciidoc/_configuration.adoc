...
=== Configuring JUnit to execute the TCK

The Jakarta Validation test harness is built atop JUnit 6 (Jupiter), and it is
JUnit that is responsible for selecting the tests to execute, the order
of execution, and reporting the results. Detailed JUnit documentation can
be found at https://junit.org/junit5/docs/current/user-guide/[junit.org].

The TCK tests are discovered automatically via classpath scanning. When using
the Maven Surefire plugin, this is configured through the `dependenciesToScan`
option, which instructs Surefire to scan the TCK artifact for test classes.
The test suite must be run unmodified for an implementation to pass the TCK.
For testing purposes it is of course ok to apply additional configuration
(see also the JUnit https://junit.org/junit5/docs/current/user-guide/[documentation])
...
JUnit provides extensive reporting information. Depending on the
build tool or IDE you use, the reporting will take a different format.
Please consult the JUnit documentation and the tool documentation for
more information.
