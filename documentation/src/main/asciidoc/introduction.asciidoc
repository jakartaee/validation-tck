...
container is that Bean Validation is part of Java EE 7 itself.
====
...
* Run and pass the test suite (see <<configuration>>). The test must be run within an EE 7
...
Edition (Java EE) 7 reference implementation (RI), aka Glassfish 4.
====